    ...
    if test -b $BLOCK_SOURCE; then
      ...
      rc=$?
        ...
        if [[ -s /tmp/filelist.txt ]]; then
            ...
            rsync -aAhHSxz --no-o --perms --no-g --no-r --exclude=lost+found --itemize-changes --info=stats2,misc2 --files-from=/tmp/filelist.txt ${SOURCE}/ rsync://127.0.0.1:$STUNNEL_LISTEN_PORT/data
        else
